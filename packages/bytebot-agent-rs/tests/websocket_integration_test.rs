...
use bytebot_agent_rs::{
    config::Config,
    websocket::{WebSocketGateway, events::ServerMessage},
    server::{create_app, AppState},
    ai::UnifiedAIService,
    database::DatabaseManager,
};
use bytebot_shared_rs::types::{Task, Message, TaskType, TaskStatus, TaskPriority, Role};
...
async fn test_websocket_server_integration() {
    ...
    websocket_gateway.emit_task_update(test_task_id, &test_task).await;
    websocket_gateway.emit_new_message(test_task_id, &test_message).await;
    websocket_gateway.emit_task_deleted(test_task_id).await;
    
    // Test broadcast methods
    websocket_gateway.broadcast_global("test_event", json!({"test": "data"})).await;
    websocket_gateway.broadcast_to_task(test_task_id, "test_event", json!({"test": "data"})).await;
}
...
async fn test_websocket_event_handlers() {
    ...
    gateway.emit_new_message("test-task-456", &test_message).await;
    gateway.emit_task_deleted("test-task-456").await;
    
    // Test broadcast methods
    gateway.broadcast_global("global_event", json!({"data": "test"})).await;
    gateway.broadcast_to_task("test-task-456", "task_event", json!({"data": "test"})).await;
}
...
async fn test_websocket_event_emitters() {
    ...
    gateway.emit_new_message("task-002", &assistant_message).await;
...
async fn test_websocket_broadcast_functionality() {
    ...
    gateway.broadcast_global("system_status", json!({
        "status": "healthy",
        "timestamp": chrono::Utc::now()
    })).await;
    
    gateway.broadcast_global("maintenance_mode", json!({
        "enabled": false,
    ...
    gateway.broadcast_to_task(task_id, "task_progress", json!({
        "progress": 50,
        "step": "Processing data"
    })).await;
    
    gateway.broadcast_to_task(task_id, "task_warning", json!({
        "warning": "Low disk space",
        "severity": "medium"
    })).await;
    
    gateway.broadcast_to_task(task_id, "task_completion", json!({
        "completed": true,
        "result": "Success"
    })).await;
}
...
async fn test_websocket_error_handling() {
    ...
    gateway.emit_task_update("non-existent-task", &non_existent_task).await;
    
    let non_existent_message = create_test_message("msg-999", "non-existent-task");
    gateway.emit_new_message("non-existent-task", &non_existent_message).await;
    
    // Test broadcasting to empty rooms (should not panic)
    gateway.broadcast_to_task("empty-room", "test_event", json!({"test": true})).await;
    
    // Test with invalid JSON data (should handle gracefully)
    gateway.broadcast_global("test_event", json!(null)).await;
    gateway.broadcast_to_task("test-task", "test_event", json!({})).await;
}
...
async fn test_websocket_message_format_compatibility() {
    ...
    let task_created_msg = ServerMessage::TaskCreated { task: test_task.clone() };
    ...
    let task_updated_msg = ServerMessage::TaskUpdated { task: test_task.clone() };
    let serialized = serde_json::to_string(&task_updated_msg).expect("Should serialize");
    assert!(serialized.contains("TaskUpdated"));
    
    let new_message_msg = ServerMessage::NewMessage { message: test_message };
...
async fn test_websocket_event_serialization() {
    use bytebot_agent_rs::websocket::events::{ServerMessage, ClientMessage};
    
    let test_task = create_test_task("serialization-test-123");
    
    // Test ServerMessage serialization
    let messages = vec![
        ServerMessage::TaskCreated { task: test_task.clone() },
        ServerMessage::TaskUpdated { task: test_task.clone() },
        ServerMessage::TaskDeleted { task_id: "serialization-test-123".to_string() },
        ServerMessage::TaskJoined { task_id: "serialization-test-123".to_string() },
        ServerMessage::TaskLeft { task_id: "serialization-test-123".to_string() },
...
async fn test_websocket_error_scenarios() {
    ...
    gateway.emit_new_message(cancel_task_id, &cancel_message).await;
}
...
async fn test_websocket_concurrent_events() {
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            ...
            gateway_clone.emit_task_update(&task_id, &updated_task).await;
    ...
    for i in 0..5 {
        let gateway_clone = gateway.clone();
        let handle = tokio::spawn(async move {
            gateway_clone.broadcast_global(
                "concurrent_test",
    ...
    for handle in broadcast_handles {
        handle.await.expect("Broadcast should complete successfully");
    }
}
...
async fn test_websocket_timeout_reliability() {
    ...
    let result = timeout(Duration::from_secs(1), async {
        ...
        gateway.broadcast_global("timeout_test", json!({"test": true})).await;
        gateway.broadcast_to_task("timeout-test-001", "timeout_test", json!({"test": true})).await;
        
        gateway.emit_task_deleted("timeout-test-001").await;
    }).await;
    
    assert!(result.is_ok(), "WebSocket operations should complete within timeout");
}